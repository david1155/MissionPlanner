...
comma2=''
steps=[]
with open(f'{basedir}test.txt', 'w') as json:
    json.write('{     "Layouts": [ ')

    for frame in range(scene.frame_start, scene.frame_end, 24):
        scene.frame_set(frame)
        scene.update()
        the_file = "pointCloud_f" + '{:03d}'.format(scene.frame_current) + ".ply"
        print(f"loading {basedir}{the_file}" + '\n')
        with open(basedir + the_file, 'w') as wFile:
            ...
            for obj in bpy.data.objects:
                ...
                comma = ','
            steps.append('{:03d}'.format(scene.frame_current))
            json.write('}, "Id": "' + '{:03d}'.format(scene.frame_current) + '" }')
            comma2 = ','
